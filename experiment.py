...
class Wifi_experiment(object):
    ...
    def _do_test(self, parameters):
        ...
        if parameters['traffic']!=0:
            # wait for wifi clients to be connected
            time.sleep(60) 
	    # run file tranfers
	...
	# set log file for each experiment
        self.logger._change_experiment('{0}.log'.format(parameters['name']))
        # change to normal netid and reset manager
        self.logger._change_netid(NET_ID)

        # wait until experiment finish
        time.sleep(TEST_TIME)

	# stop logging to finish the experiment
	self.logger._change_experiment('one.log')

	# change netid to reset all the network
